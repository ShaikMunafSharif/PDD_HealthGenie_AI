// HealthGenie AI - Real-time Doctor Service & API Key Management

export async function fetchNearbyDoctors(lat, lng, specialty = '', radius = 5000, query = '') {
  try {
    const res = await fetch(`/api/doctors/nearby?lat=${lat}&lng=${lng}&radius=${radius}&specialty=${encodeURIComponent(specialty)}&query=${encodeURIComponent(query)}`);
...
export async function saveAPIKeys({ googlePlacesKey, geminiKey }) {
  try {
    const res = await fetch('/api/config/keys', {
      method: 'POST',
...
function getClientFallbackDoctors(specialty = '') {
  const doctors = [
